
/app/bin_gccgcc10_O0/trimming:     file format elf64-x86-64


Disassembly of section .init:

0000000000001000 <_init>:
    1000:	f3 0f 1e fa          	endbr64 
    1004:	48 83 ec 08          	sub    $0x8,%rsp
    1008:	48 8b 05 d9 2f 00 00 	mov    0x2fd9(%rip),%rax        # 3fe8 <__gmon_start__>
    100f:	48 85 c0             	test   %rax,%rax
    1012:	74 02                	je     1016 <_init+0x16>
    1014:	ff d0                	callq  *%rax
    1016:	48 83 c4 08          	add    $0x8,%rsp
    101a:	c3                   	retq   

Disassembly of section .plt:

0000000000001020 <.plt>:
    1020:	ff 35 7a 2f 00 00    	pushq  0x2f7a(%rip)        # 3fa0 <_GLOBAL_OFFSET_TABLE_+0x8>
    1026:	f2 ff 25 7b 2f 00 00 	bnd jmpq *0x2f7b(%rip)        # 3fa8 <_GLOBAL_OFFSET_TABLE_+0x10>
    102d:	0f 1f 00             	nopl   (%rax)
    1030:	f3 0f 1e fa          	endbr64 
    1034:	68 00 00 00 00       	pushq  $0x0
    1039:	f2 e9 e1 ff ff ff    	bnd jmpq 1020 <.plt>
    103f:	90                   	nop
    1040:	f3 0f 1e fa          	endbr64 
    1044:	68 01 00 00 00       	pushq  $0x1
    1049:	f2 e9 d1 ff ff ff    	bnd jmpq 1020 <.plt>
    104f:	90                   	nop
    1050:	f3 0f 1e fa          	endbr64 
    1054:	68 02 00 00 00       	pushq  $0x2
    1059:	f2 e9 c1 ff ff ff    	bnd jmpq 1020 <.plt>
    105f:	90                   	nop
    1060:	f3 0f 1e fa          	endbr64 
    1064:	68 03 00 00 00       	pushq  $0x3
    1069:	f2 e9 b1 ff ff ff    	bnd jmpq 1020 <.plt>
    106f:	90                   	nop
    1070:	f3 0f 1e fa          	endbr64 
    1074:	68 04 00 00 00       	pushq  $0x4
    1079:	f2 e9 a1 ff ff ff    	bnd jmpq 1020 <.plt>
    107f:	90                   	nop

Disassembly of section .plt.got:

0000000000001080 <__cxa_finalize@plt>:
    1080:	f3 0f 1e fa          	endbr64 
    1084:	f2 ff 25 6d 2f 00 00 	bnd jmpq *0x2f6d(%rip)        # 3ff8 <__cxa_finalize@GLIBC_2.2.5>
    108b:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

Disassembly of section .plt.sec:

0000000000001090 <__stack_chk_fail@plt>:
    1090:	f3 0f 1e fa          	endbr64 
    1094:	f2 ff 25 15 2f 00 00 	bnd jmpq *0x2f15(%rip)        # 3fb0 <__stack_chk_fail@GLIBC_2.4>
    109b:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

00000000000010a0 <printf@plt>:
    10a0:	f3 0f 1e fa          	endbr64 
    10a4:	f2 ff 25 0d 2f 00 00 	bnd jmpq *0x2f0d(%rip)        # 3fb8 <printf@GLIBC_2.2.5>
    10ab:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

00000000000010b0 <malloc@plt>:
    10b0:	f3 0f 1e fa          	endbr64 
    10b4:	f2 ff 25 05 2f 00 00 	bnd jmpq *0x2f05(%rip)        # 3fc0 <malloc@GLIBC_2.2.5>
    10bb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

00000000000010c0 <exit@plt>:
    10c0:	f3 0f 1e fa          	endbr64 
    10c4:	f2 ff 25 fd 2e 00 00 	bnd jmpq *0x2efd(%rip)        # 3fc8 <exit@GLIBC_2.2.5>
    10cb:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

00000000000010d0 <fwrite@plt>:
    10d0:	f3 0f 1e fa          	endbr64 
    10d4:	f2 ff 25 f5 2e 00 00 	bnd jmpq *0x2ef5(%rip)        # 3fd0 <fwrite@GLIBC_2.2.5>
    10db:	0f 1f 44 00 00       	nopl   0x0(%rax,%rax,1)

Disassembly of section .text:

00000000000010e0 <_start>:
    10e0:	f3 0f 1e fa          	endbr64 
    10e4:	31 ed                	xor    %ebp,%ebp
    10e6:	49 89 d1             	mov    %rdx,%r9
    10e9:	5e                   	pop    %rsi
    10ea:	48 89 e2             	mov    %rsp,%rdx
    10ed:	48 83 e4 f0          	and    $0xfffffffffffffff0,%rsp
    10f1:	50                   	push   %rax
    10f2:	54                   	push   %rsp
    10f3:	4c 8d 05 b6 04 00 00 	lea    0x4b6(%rip),%r8        # 15b0 <__libc_csu_fini>
    10fa:	48 8d 0d 3f 04 00 00 	lea    0x43f(%rip),%rcx        # 1540 <__libc_csu_init>
    1101:	48 8d 3d 21 03 00 00 	lea    0x321(%rip),%rdi        # 1429 <main>
    1108:	ff 15 d2 2e 00 00    	callq  *0x2ed2(%rip)        # 3fe0 <__libc_start_main@GLIBC_2.2.5>
    110e:	f4                   	hlt    
    110f:	90                   	nop

0000000000001110 <deregister_tm_clones>:
    1110:	48 8d 3d f9 2e 00 00 	lea    0x2ef9(%rip),%rdi        # 4010 <__TMC_END__>
    1117:	48 8d 05 f2 2e 00 00 	lea    0x2ef2(%rip),%rax        # 4010 <__TMC_END__>
    111e:	48 39 f8             	cmp    %rdi,%rax
    1121:	74 15                	je     1138 <deregister_tm_clones+0x28>
    1123:	48 8b 05 ae 2e 00 00 	mov    0x2eae(%rip),%rax        # 3fd8 <_ITM_deregisterTMCloneTable>
    112a:	48 85 c0             	test   %rax,%rax
    112d:	74 09                	je     1138 <deregister_tm_clones+0x28>
    112f:	ff e0                	jmpq   *%rax
    1131:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)
    1138:	c3                   	retq   
    1139:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)

0000000000001140 <register_tm_clones>:
    1140:	48 8d 3d c9 2e 00 00 	lea    0x2ec9(%rip),%rdi        # 4010 <__TMC_END__>
    1147:	48 8d 35 c2 2e 00 00 	lea    0x2ec2(%rip),%rsi        # 4010 <__TMC_END__>
    114e:	48 29 fe             	sub    %rdi,%rsi
    1151:	48 89 f0             	mov    %rsi,%rax
    1154:	48 c1 ee 3f          	shr    $0x3f,%rsi
    1158:	48 c1 f8 03          	sar    $0x3,%rax
    115c:	48 01 c6             	add    %rax,%rsi
    115f:	48 d1 fe             	sar    %rsi
    1162:	74 14                	je     1178 <register_tm_clones+0x38>
    1164:	48 8b 05 85 2e 00 00 	mov    0x2e85(%rip),%rax        # 3ff0 <_ITM_registerTMCloneTable>
    116b:	48 85 c0             	test   %rax,%rax
    116e:	74 08                	je     1178 <register_tm_clones+0x38>
    1170:	ff e0                	jmpq   *%rax
    1172:	66 0f 1f 44 00 00    	nopw   0x0(%rax,%rax,1)
    1178:	c3                   	retq   
    1179:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)

0000000000001180 <__do_global_dtors_aux>:
    1180:	f3 0f 1e fa          	endbr64 
    1184:	80 3d 9d 2e 00 00 00 	cmpb   $0x0,0x2e9d(%rip)        # 4028 <completed.0>
    118b:	75 2b                	jne    11b8 <__do_global_dtors_aux+0x38>
    118d:	55                   	push   %rbp
    118e:	48 83 3d 62 2e 00 00 	cmpq   $0x0,0x2e62(%rip)        # 3ff8 <__cxa_finalize@GLIBC_2.2.5>
    1195:	00 
    1196:	48 89 e5             	mov    %rsp,%rbp
    1199:	74 0c                	je     11a7 <__do_global_dtors_aux+0x27>
    119b:	48 8b 3d 66 2e 00 00 	mov    0x2e66(%rip),%rdi        # 4008 <__dso_handle>
    11a2:	e8 d9 fe ff ff       	callq  1080 <__cxa_finalize@plt>
    11a7:	e8 64 ff ff ff       	callq  1110 <deregister_tm_clones>
    11ac:	c6 05 75 2e 00 00 01 	movb   $0x1,0x2e75(%rip)        # 4028 <completed.0>
    11b3:	5d                   	pop    %rbp
    11b4:	c3                   	retq   
    11b5:	0f 1f 00             	nopl   (%rax)
    11b8:	c3                   	retq   
    11b9:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)

00000000000011c0 <frame_dummy>:
    11c0:	f3 0f 1e fa          	endbr64 
    11c4:	e9 77 ff ff ff       	jmpq   1140 <register_tm_clones>

00000000000011c9 <left>:
    11c9:	f3 0f 1e fa          	endbr64 
    11cd:	55                   	push   %rbp
    11ce:	48 89 e5             	mov    %rsp,%rbp
    11d1:	48 83 ec 20          	sub    $0x20,%rsp
    11d5:	48 89 7d e8          	mov    %rdi,-0x18(%rbp)
    11d9:	89 75 e4             	mov    %esi,-0x1c(%rbp)
    11dc:	8b 45 e4             	mov    -0x1c(%rbp),%eax
    11df:	48 98                	cltq   
    11e1:	48 83 c0 01          	add    $0x1,%rax
    11e5:	48 89 c7             	mov    %rax,%rdi
    11e8:	e8 c3 fe ff ff       	callq  10b0 <malloc@plt>
    11ed:	48 89 45 f8          	mov    %rax,-0x8(%rbp)
    11f1:	48 83 7d f8 00       	cmpq   $0x0,-0x8(%rbp)
    11f6:	75 2a                	jne    1222 <left+0x59>
    11f8:	48 8b 05 21 2e 00 00 	mov    0x2e21(%rip),%rax        # 4020 <stderr@@GLIBC_2.2.5>
    11ff:	48 89 c1             	mov    %rax,%rcx
    1202:	ba 18 00 00 00       	mov    $0x18,%edx
    1207:	be 01 00 00 00       	mov    $0x1,%esi
    120c:	48 8d 3d f1 0d 00 00 	lea    0xdf1(%rip),%rdi        # 2004 <_IO_stdin_used+0x4>
    1213:	e8 b8 fe ff ff       	callq  10d0 <fwrite@plt>
    1218:	bf 01 00 00 00       	mov    $0x1,%edi
    121d:	e8 9e fe ff ff       	callq  10c0 <exit@plt>
    1222:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%rbp)
    1229:	eb 37                	jmp    1262 <left+0x99>
    122b:	8b 45 f4             	mov    -0xc(%rbp),%eax
    122e:	48 63 d0             	movslq %eax,%rdx
    1231:	48 8b 45 e8          	mov    -0x18(%rbp),%rax
    1235:	48 01 d0             	add    %rdx,%rax
    1238:	0f b6 00             	movzbl (%rax),%eax
    123b:	84 c0                	test   %al,%al
    123d:	74 2d                	je     126c <left+0xa3>
    123f:	8b 45 f4             	mov    -0xc(%rbp),%eax
    1242:	48 63 d0             	movslq %eax,%rdx
    1245:	48 8b 45 e8          	mov    -0x18(%rbp),%rax
    1249:	48 01 d0             	add    %rdx,%rax
    124c:	8b 55 f4             	mov    -0xc(%rbp),%edx
    124f:	48 63 ca             	movslq %edx,%rcx
    1252:	48 8b 55 f8          	mov    -0x8(%rbp),%rdx
    1256:	48 01 ca             	add    %rcx,%rdx
    1259:	0f b6 00             	movzbl (%rax),%eax
    125c:	88 02                	mov    %al,(%rdx)
    125e:	83 45 f4 01          	addl   $0x1,-0xc(%rbp)
    1262:	8b 45 f4             	mov    -0xc(%rbp),%eax
    1265:	3b 45 e4             	cmp    -0x1c(%rbp),%eax
    1268:	7c c1                	jl     122b <left+0x62>
    126a:	eb 01                	jmp    126d <left+0xa4>
    126c:	90                   	nop
    126d:	8b 45 f4             	mov    -0xc(%rbp),%eax
    1270:	48 63 d0             	movslq %eax,%rdx
    1273:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    1277:	48 01 d0             	add    %rdx,%rax
    127a:	c6 00 00             	movb   $0x0,(%rax)
    127d:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    1281:	c9                   	leaveq 
    1282:	c3                   	retq   

0000000000001283 <right>:
    1283:	f3 0f 1e fa          	endbr64 
    1287:	55                   	push   %rbp
    1288:	48 89 e5             	mov    %rsp,%rbp
    128b:	48 83 ec 30          	sub    $0x30,%rsp
    128f:	48 89 7d d8          	mov    %rdi,-0x28(%rbp)
    1293:	89 75 d4             	mov    %esi,-0x2c(%rbp)
    1296:	8b 45 d4             	mov    -0x2c(%rbp),%eax
    1299:	48 98                	cltq   
    129b:	48 83 c0 01          	add    $0x1,%rax
    129f:	48 89 c7             	mov    %rax,%rdi
    12a2:	e8 09 fe ff ff       	callq  10b0 <malloc@plt>
    12a7:	48 89 45 f8          	mov    %rax,-0x8(%rbp)
    12ab:	48 83 7d f8 00       	cmpq   $0x0,-0x8(%rbp)
    12b0:	75 2a                	jne    12dc <right+0x59>
    12b2:	48 8b 05 67 2d 00 00 	mov    0x2d67(%rip),%rax        # 4020 <stderr@@GLIBC_2.2.5>
    12b9:	48 89 c1             	mov    %rax,%rcx
    12bc:	ba 18 00 00 00       	mov    $0x18,%edx
    12c1:	be 01 00 00 00       	mov    $0x1,%esi
    12c6:	48 8d 3d 37 0d 00 00 	lea    0xd37(%rip),%rdi        # 2004 <_IO_stdin_used+0x4>
    12cd:	e8 fe fd ff ff       	callq  10d0 <fwrite@plt>
    12d2:	bf 01 00 00 00       	mov    $0x1,%edi
    12d7:	e8 e4 fd ff ff       	callq  10c0 <exit@plt>
    12dc:	48 8b 45 d8          	mov    -0x28(%rbp),%rax
    12e0:	48 89 45 f0          	mov    %rax,-0x10(%rbp)
    12e4:	eb 05                	jmp    12eb <right+0x68>
    12e6:	48 83 45 f0 01       	addq   $0x1,-0x10(%rbp)
    12eb:	48 8b 45 f0          	mov    -0x10(%rbp),%rax
    12ef:	0f b6 00             	movzbl (%rax),%eax
    12f2:	84 c0                	test   %al,%al
    12f4:	75 f0                	jne    12e6 <right+0x63>
    12f6:	8b 45 d4             	mov    -0x2c(%rbp),%eax
    12f9:	48 98                	cltq   
    12fb:	48 f7 d8             	neg    %rax
    12fe:	48 01 45 f0          	add    %rax,-0x10(%rbp)
    1302:	48 8b 45 f0          	mov    -0x10(%rbp),%rax
    1306:	48 3b 45 d8          	cmp    -0x28(%rbp),%rax
    130a:	73 0a                	jae    1316 <right+0x93>
    130c:	bf 01 00 00 00       	mov    $0x1,%edi
    1311:	e8 aa fd ff ff       	callq  10c0 <exit@plt>
    1316:	c7 45 ec 00 00 00 00 	movl   $0x0,-0x14(%rbp)
    131d:	eb 23                	jmp    1342 <right+0xbf>
    131f:	8b 45 ec             	mov    -0x14(%rbp),%eax
    1322:	48 63 d0             	movslq %eax,%rdx
    1325:	48 8b 45 f0          	mov    -0x10(%rbp),%rax
    1329:	48 01 d0             	add    %rdx,%rax
    132c:	8b 55 ec             	mov    -0x14(%rbp),%edx
    132f:	48 63 ca             	movslq %edx,%rcx
    1332:	48 8b 55 f8          	mov    -0x8(%rbp),%rdx
    1336:	48 01 ca             	add    %rcx,%rdx
    1339:	0f b6 00             	movzbl (%rax),%eax
    133c:	88 02                	mov    %al,(%rdx)
    133e:	83 45 ec 01          	addl   $0x1,-0x14(%rbp)
    1342:	8b 45 ec             	mov    -0x14(%rbp),%eax
    1345:	3b 45 d4             	cmp    -0x2c(%rbp),%eax
    1348:	7c d5                	jl     131f <right+0x9c>
    134a:	8b 45 ec             	mov    -0x14(%rbp),%eax
    134d:	48 63 d0             	movslq %eax,%rdx
    1350:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    1354:	48 01 d0             	add    %rdx,%rax
    1357:	c6 00 00             	movb   $0x0,(%rax)
    135a:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    135e:	c9                   	leaveq 
    135f:	c3                   	retq   

0000000000001360 <mid>:
    1360:	f3 0f 1e fa          	endbr64 
    1364:	55                   	push   %rbp
    1365:	48 89 e5             	mov    %rsp,%rbp
    1368:	48 83 ec 20          	sub    $0x20,%rsp
    136c:	48 89 7d e8          	mov    %rdi,-0x18(%rbp)
    1370:	89 75 e4             	mov    %esi,-0x1c(%rbp)
    1373:	89 55 e0             	mov    %edx,-0x20(%rbp)
    1376:	8b 45 e0             	mov    -0x20(%rbp),%eax
    1379:	48 98                	cltq   
    137b:	48 83 c0 01          	add    $0x1,%rax
    137f:	48 89 c7             	mov    %rax,%rdi
    1382:	e8 29 fd ff ff       	callq  10b0 <malloc@plt>
    1387:	48 89 45 f8          	mov    %rax,-0x8(%rbp)
    138b:	48 83 7d f8 00       	cmpq   $0x0,-0x8(%rbp)
    1390:	75 2a                	jne    13bc <mid+0x5c>
    1392:	48 8b 05 87 2c 00 00 	mov    0x2c87(%rip),%rax        # 4020 <stderr@@GLIBC_2.2.5>
    1399:	48 89 c1             	mov    %rax,%rcx
    139c:	ba 18 00 00 00       	mov    $0x18,%edx
    13a1:	be 01 00 00 00       	mov    $0x1,%esi
    13a6:	48 8d 3d 57 0c 00 00 	lea    0xc57(%rip),%rdi        # 2004 <_IO_stdin_used+0x4>
    13ad:	e8 1e fd ff ff       	callq  10d0 <fwrite@plt>
    13b2:	bf 01 00 00 00       	mov    $0x1,%edi
    13b7:	e8 04 fd ff ff       	callq  10c0 <exit@plt>
    13bc:	c7 45 f4 00 00 00 00 	movl   $0x0,-0xc(%rbp)
    13c3:	eb 43                	jmp    1408 <mid+0xa8>
    13c5:	8b 45 e4             	mov    -0x1c(%rbp),%eax
    13c8:	48 63 d0             	movslq %eax,%rdx
    13cb:	8b 45 f4             	mov    -0xc(%rbp),%eax
    13ce:	48 98                	cltq   
    13d0:	48 01 d0             	add    %rdx,%rax
    13d3:	48 8d 50 ff          	lea    -0x1(%rax),%rdx
    13d7:	48 8b 45 e8          	mov    -0x18(%rbp),%rax
    13db:	48 01 d0             	add    %rdx,%rax
    13de:	8b 55 f4             	mov    -0xc(%rbp),%edx
    13e1:	48 63 ca             	movslq %edx,%rcx
    13e4:	48 8b 55 f8          	mov    -0x8(%rbp),%rdx
    13e8:	48 01 ca             	add    %rcx,%rdx
    13eb:	0f b6 00             	movzbl (%rax),%eax
    13ee:	88 02                	mov    %al,(%rdx)
    13f0:	8b 45 f4             	mov    -0xc(%rbp),%eax
    13f3:	48 63 d0             	movslq %eax,%rdx
    13f6:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    13fa:	48 01 d0             	add    %rdx,%rax
    13fd:	0f b6 00             	movzbl (%rax),%eax
    1400:	84 c0                	test   %al,%al
    1402:	74 0e                	je     1412 <mid+0xb2>
    1404:	83 45 f4 01          	addl   $0x1,-0xc(%rbp)
    1408:	8b 45 f4             	mov    -0xc(%rbp),%eax
    140b:	3b 45 e0             	cmp    -0x20(%rbp),%eax
    140e:	7c b5                	jl     13c5 <mid+0x65>
    1410:	eb 01                	jmp    1413 <mid+0xb3>
    1412:	90                   	nop
    1413:	8b 45 f4             	mov    -0xc(%rbp),%eax
    1416:	48 63 d0             	movslq %eax,%rdx
    1419:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    141d:	48 01 d0             	add    %rdx,%rax
    1420:	c6 00 00             	movb   $0x0,(%rax)
    1423:	48 8b 45 f8          	mov    -0x8(%rbp),%rax
    1427:	c9                   	leaveq 
    1428:	c3                   	retq   

0000000000001429 <main>:
    1429:	f3 0f 1e fa          	endbr64 
    142d:	55                   	push   %rbp
    142e:	48 89 e5             	mov    %rsp,%rbp
    1431:	48 83 ec 30          	sub    $0x30,%rsp
    1435:	64 48 8b 04 25 28 00 	mov    %fs:0x28,%rax
    143c:	00 00 
    143e:	48 89 45 f8          	mov    %rax,-0x8(%rbp)
    1442:	31 c0                	xor    %eax,%eax
    1444:	48 b8 53 6c 69 63 65 	movabs $0x656d206563696c53,%rax
    144b:	20 6d 65 
    144e:	48 ba 2c 20 64 69 63 	movabs $0x6d2065636964202c,%rdx
    1455:	65 20 6d 
    1458:	48 89 45 d0          	mov    %rax,-0x30(%rbp)
    145c:	48 89 55 d8          	mov    %rdx,-0x28(%rbp)
    1460:	48 b8 65 2c 20 6d 61 	movabs $0x20656b616d202c65,%rax
    1467:	6b 65 20 
    146a:	48 ba 4a 75 6c 69 65 	movabs $0x656e6e65696c754a,%rdx
    1471:	6e 6e 65 
    1474:	48 89 45 e0          	mov    %rax,-0x20(%rbp)
    1478:	48 89 55 e8          	mov    %rdx,-0x18(%rbp)
    147c:	48 b8 20 66 72 69 65 	movabs $0x21736569726620,%rax
    1483:	73 21 00 
    1486:	48 89 45 f0          	mov    %rax,-0x10(%rbp)
    148a:	48 8d 45 d0          	lea    -0x30(%rbp),%rax
    148e:	48 89 c6             	mov    %rax,%rsi
    1491:	48 8d 3d 85 0b 00 00 	lea    0xb85(%rip),%rdi        # 201d <_IO_stdin_used+0x1d>
    1498:	b8 00 00 00 00       	mov    $0x0,%eax
    149d:	e8 fe fb ff ff       	callq  10a0 <printf@plt>
    14a2:	48 8d 45 d0          	lea    -0x30(%rbp),%rax
    14a6:	be 08 00 00 00       	mov    $0x8,%esi
    14ab:	48 89 c7             	mov    %rax,%rdi
    14ae:	e8 16 fd ff ff       	callq  11c9 <left>
    14b3:	48 89 c2             	mov    %rax,%rdx
    14b6:	be 08 00 00 00       	mov    $0x8,%esi
    14bb:	48 8d 3d 70 0b 00 00 	lea    0xb70(%rip),%rdi        # 2032 <_IO_stdin_used+0x32>
    14c2:	b8 00 00 00 00       	mov    $0x0,%eax
    14c7:	e8 d4 fb ff ff       	callq  10a0 <printf@plt>
    14cc:	48 8d 45 d0          	lea    -0x30(%rbp),%rax
    14d0:	ba 07 00 00 00       	mov    $0x7,%edx
    14d5:	be 0b 00 00 00       	mov    $0xb,%esi
    14da:	48 89 c7             	mov    %rax,%rdi
    14dd:	e8 7e fe ff ff       	callq  1360 <mid>
    14e2:	48 89 c2             	mov    %rax,%rdx
    14e5:	be 07 00 00 00       	mov    $0x7,%esi
    14ea:	48 8d 3d 59 0b 00 00 	lea    0xb59(%rip),%rdi        # 204a <_IO_stdin_used+0x4a>
    14f1:	b8 00 00 00 00       	mov    $0x0,%eax
    14f6:	e8 a5 fb ff ff       	callq  10a0 <printf@plt>
    14fb:	48 8d 45 d0          	lea    -0x30(%rbp),%rax
    14ff:	be 14 00 00 00       	mov    $0x14,%esi
    1504:	48 89 c7             	mov    %rax,%rdi
    1507:	e8 77 fd ff ff       	callq  1283 <right>
    150c:	48 89 c2             	mov    %rax,%rdx
    150f:	be 14 00 00 00       	mov    $0x14,%esi
    1514:	48 8d 3d 49 0b 00 00 	lea    0xb49(%rip),%rdi        # 2064 <_IO_stdin_used+0x64>
    151b:	b8 00 00 00 00       	mov    $0x0,%eax
    1520:	e8 7b fb ff ff       	callq  10a0 <printf@plt>
    1525:	b8 00 00 00 00       	mov    $0x0,%eax
    152a:	48 8b 4d f8          	mov    -0x8(%rbp),%rcx
    152e:	64 48 2b 0c 25 28 00 	sub    %fs:0x28,%rcx
    1535:	00 00 
    1537:	74 05                	je     153e <main+0x115>
    1539:	e8 52 fb ff ff       	callq  1090 <__stack_chk_fail@plt>
    153e:	c9                   	leaveq 
    153f:	c3                   	retq   

0000000000001540 <__libc_csu_init>:
    1540:	f3 0f 1e fa          	endbr64 
    1544:	41 57                	push   %r15
    1546:	4c 8d 3d 4b 28 00 00 	lea    0x284b(%rip),%r15        # 3d98 <__frame_dummy_init_array_entry>
    154d:	41 56                	push   %r14
    154f:	49 89 d6             	mov    %rdx,%r14
    1552:	41 55                	push   %r13
    1554:	49 89 f5             	mov    %rsi,%r13
    1557:	41 54                	push   %r12
    1559:	41 89 fc             	mov    %edi,%r12d
    155c:	55                   	push   %rbp
    155d:	48 8d 2d 3c 28 00 00 	lea    0x283c(%rip),%rbp        # 3da0 <__do_global_dtors_aux_fini_array_entry>
    1564:	53                   	push   %rbx
    1565:	4c 29 fd             	sub    %r15,%rbp
    1568:	48 83 ec 08          	sub    $0x8,%rsp
    156c:	e8 8f fa ff ff       	callq  1000 <_init>
    1571:	48 c1 fd 03          	sar    $0x3,%rbp
    1575:	74 1f                	je     1596 <__libc_csu_init+0x56>
    1577:	31 db                	xor    %ebx,%ebx
    1579:	0f 1f 80 00 00 00 00 	nopl   0x0(%rax)
    1580:	4c 89 f2             	mov    %r14,%rdx
    1583:	4c 89 ee             	mov    %r13,%rsi
    1586:	44 89 e7             	mov    %r12d,%edi
    1589:	41 ff 14 df          	callq  *(%r15,%rbx,8)
    158d:	48 83 c3 01          	add    $0x1,%rbx
    1591:	48 39 dd             	cmp    %rbx,%rbp
    1594:	75 ea                	jne    1580 <__libc_csu_init+0x40>
    1596:	48 83 c4 08          	add    $0x8,%rsp
    159a:	5b                   	pop    %rbx
    159b:	5d                   	pop    %rbp
    159c:	41 5c                	pop    %r12
    159e:	41 5d                	pop    %r13
    15a0:	41 5e                	pop    %r14
    15a2:	41 5f                	pop    %r15
    15a4:	c3                   	retq   
    15a5:	66 66 2e 0f 1f 84 00 	data16 nopw %cs:0x0(%rax,%rax,1)
    15ac:	00 00 00 00 

00000000000015b0 <__libc_csu_fini>:
    15b0:	f3 0f 1e fa          	endbr64 
    15b4:	c3                   	retq   

Disassembly of section .fini:

00000000000015b8 <_fini>:
    15b8:	f3 0f 1e fa          	endbr64 
    15bc:	48 83 ec 08          	sub    $0x8,%rsp
    15c0:	48 83 c4 08          	add    $0x8,%rsp
    15c4:	c3                   	retq   
